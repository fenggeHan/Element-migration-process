...
def main():
    ...
    if not st.session_state.current_scene:
        ...
        if not sim_results:
            ...
            with col4:
                # 第1处修改：强制使用当前选中场景的名称（核心修复点）
                st.metric("场景名称", scene_options[selected_scene_key])
            ...
            with col_excel:
                try:
                    ...
                    if excel_bytes:
                        # 第2处修改：导出文件名使用当前选中场景的名称（核心修复点）
                        scene_name = scene_options[selected_scene_key].replace(" ", "_")
                        st.download_button(
                            label="导出Excel数据",
